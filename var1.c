        ...
        for (k = 0; k < size; k++) {
            ...
            for (i = 0; i < part.size; i++) {
                for (j = 0; j < N; j++) {
                    buff[i * N + j] = 0;
                    for (l = 0; l < N; l++) {
                        buff[i * N + j] += A[N * l + j] * A[N * l + i + part.id];
                    }
                }
            }
            if (k != rank) {
...
void buildRightVector(double *vector, const double *coefficientMatrix, int N, int partSize, int partId) {
    ...
    dot_mv(coefficientMatrix, u, partSize, N, vector + partId);
    share(vector, N);
...
int main(int argc, char **argv) {
    ...
    while (1) {
        dot_mv(A, z, part.size, N, A_mul_z + part.id);
	share(A_mul_z, N);
    ...
    dot_mv(A, x, part.size, N, A_mul_z + part.id);
